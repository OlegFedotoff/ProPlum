-- DROP FUNCTION fw.f_get_pxf_partition(int8);
...
BEGIN
  ...
  perform fw.f_write_log(
     ...
     select li.extraction_type, coalesce(column_name_mapping->>ob.bdate_field,ob.bdate_field),coalesce(column_name_mapping->>ob.delta_field,ob.delta_field), coalesce(li.extraction_from,'1980-01-01'::date), coalesce(li.extraction_to,'2100-01-01'::date),
            ob.connect_string, ob.column_name_mapping, ob.delta_field_type, ob.bdate_field_type, ob.delta_field_format, ob.bdate_field_format, ob.load_method, ob.bdate_safety_period
     from fw.load_info li 
      join fw.objects ob on ob.object_id = li.object_id
     where li.load_id = p_load_id
    into v_extraction_type, v_bdate_field, v_delta_field,v_extraction_from,v_extraction_to, v_conn_string, v_column_map, v_delta_field_type, v_bdate_field_type, v_delta_field_format, v_bdate_field_format, v_load_method, v_bdate_safety_period;
   ...
   case 
	   when v_extraction_type in ('FULL') and v_delta_field is null then
	    v_part_string = '';
	   when v_extraction_type = 'FULL' AND ( v_load_method = 'pxf' OR v_load_method = 'PXF')
	    and v_delta_field is not null
	    and v_extraction_from is not null
	    and v_extraction_to is not null
	    and v_bdate_safety_period is not null
	    and position('&PARTITION_BY' in v_conn_string) = 0
	   then
         if v_range <= 0 then
           v_part_string = '';
         else
          if v_bdate_safety_period in ('1 day','1 mon','1 year') and v_delta_field is not null
           then
            v_bdate_safety_period = replace(replace(replace(v_bdate_safety_period,' day',':day'),' mon',':month'),' year',':year');
            v_part_string = '&PARTITION_BY='||v_delta_field||':date&RANGE='||v_extraction_from||':'||v_extraction_to||'&INTERVAL='||v_bdate_safety_period;
           else

-- DROP FUNCTION fw.f_write_chain_log(text, text, int8);

CREATE OR REPLACE FUNCTION fw.f_write_chain_log(p_log_type text, p_log_message text, p_instance_id int8)
	RETURNS void
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
BEGIN
  ...
  v_sql := 'INSERT INTO fw.chains_log(log_id, instance_id, log_timestamp, log_type, log_msg) 
            VALUES ( '||nextval('fw.chain_log_id_seq')||' , '||coalesce(p_instance_id, 0)||', current_timestamp,'''||v_log_type||''', '||coalesce(''''||v_log_message||'''','''empty''')||');';

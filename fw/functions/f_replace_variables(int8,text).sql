-- DROP FUNCTION fw.f_replace_variables(int8, text);

CREATE OR REPLACE FUNCTION fw.f_replace_variables(p_load_id int8, p_string text)
	RETURNS text
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
DECLARE
  v_location    text := 'fw.f_replace_variables';
  v_table_name  text; 
  v_sql         text;
...
BEGIN
  v_sql := 'select ob.object_name, 
                   ...
                   li.extraction_to, 
                   ob.object_id 
            from fw.load_info li, fw.objects ob where li.object_id = ob.object_id and li.load_id = ' ||p_load_id;
  execute v_sql into v_table_name, v_extraction_type, v_load_type, v_load_from, v_load_to, v_extr_from, v_extr_to, v_object_id;
  v_sql = replace(replace(replace(replace(replace(replace(replace(replace(replace(p_string,

...
AS $$
	...
	/*Copy of fw.f_gen_load_id with INSERTs executed via dblink*/
DECLARE
	v_location           text := 'fw.f_gen_load_id_dblink';
	v_object_id          int8;
...
BEGIN
	...
	-- server for dblink inserts
	v_server = fw.f_get_constant('c_log_fdw_server');

	v_load_id = fw.f_get_load_id_new_or_inproc( --find existing load_id
...
GRANT EXECUTE ON FUNCTION fw.f_gen_load_id_dblink(

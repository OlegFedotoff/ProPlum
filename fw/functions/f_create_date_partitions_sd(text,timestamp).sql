-- DROP FUNCTION fw.f_create_date_partitions_sd(text, timestamp);

CREATE OR REPLACE FUNCTION fw.f_create_date_partitions_sd(p_table_name text, p_partition_value timestamp)
	RETURNS void
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
AS $$
    ...
    * Sapiens Solutions 
    * 2023*/
/*Function create new partition for table with SECURITY DEFINER*/
DECLARE
  v_location            text := 'fw.f_create_date_partitions_sd';
...
-- ALTER FUNCTION fw.f_create_date_partitions_sd(text, timestamp) OWNER TO komus_dba;

GRANT EXECUTE ON FUNCTION fw.f_create_date_partitions_sd(text, timestamp) TO role_data_loader;

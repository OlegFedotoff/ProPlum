-- DROP FUNCTION fw.f_update_columns(int8, text, _text, text, text, bool);

CREATE OR REPLACE FUNCTION fw.f_update_columns(p_load_id int8, p_table_from_name text, p_column_list _text, p_table_to_name text DEFAULT NULL::text, p_date_field text DEFAULT NULL::text, p_terminate_lock bool DEFAULT false)
	RETURNS bool
	LANGUAGE plpgsql
	VOLATILE
AS $$
 ...
 * p_terminate_lock - true - terminate lock on target table, false - not terminate lock on target table
 */
declare
	v_location text := 'fw.f_update_columns';
...
begin
	...
	if p_terminate_lock then
	    PERFORM fw.f_terminate_lock(v_table_to_name);
	end if;
    ...
    v_schema_name = fw.f_get_table_schema(v_table_to_name);
    v_schema_name = replace(replace(replace(v_schema_name,'src_',''),'stg_',''),'load_','');
    v_schema_name = coalesce(fw.f_get_constant('c_stg_table_schema'),'stg_')||v_schema_name;-- temp table schema name
	perform fw.f_write_log(
		p_log_type    := 'INFO', 
	...
	v_sql = 
	 fw.f_replace_variables(p_load_id,
	 'select '||
    ...
    v_cnt = fw.f_create_obj_sql(
     ...
     if flag_update then
		  ...
		  perform fw.f_analyze_table(v_table_to_name||'('||array_to_string(p_column_list,',')||')');
		  -- max delta value in load table for load_id
		  v_extr_to = coalesce(fw.f_get_max_value(

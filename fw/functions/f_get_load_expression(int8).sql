-- DROP FUNCTION fw.f_get_load_expression(int8);
...
BEGIN
  ...
  v_tmp_table_name = fw.f_get_delta_table_name(p_load_id := p_load_id);
   ...
   select 'select '|| string_agg(coalesce(v_transform->>c.column_name,c.column_name)||'' || case when upper(c.data_type) = 'ARRAY' and c.udt_name = '_text' then '::_text' when upper(c.data_type) = 'ARRAY' and c.udt_name = '_int8' then '::_int8' else '' end ||' "'|| c.column_name ||'"'
  --case when c.column_name = 'user' then '"user"' else c.column_name end
  ,',' order by c.ordinal_position) ||' from '||v_ext_table_name from information_schema.columns c 

-- DROP FUNCTION fw.f_merge_tables(text, text, text, _text, text, bool);

CREATE OR REPLACE FUNCTION fw.f_merge_tables(p_table_from_name text, p_table_to_name text, p_where text, p_merge_key _text, p_trg_table text DEFAULT NULL::text, p_delete_duplicates bool DEFAULT false)
	RETURNS int8
	LANGUAGE plpgsql
	VOLATILE
AS $$

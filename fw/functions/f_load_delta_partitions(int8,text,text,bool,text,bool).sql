-- DROP FUNCTION fw.f_load_delta_partitions(int8, text, text, bool, text, bool);

CREATE OR REPLACE FUNCTION fw.f_load_delta_partitions(p_load_id int8, p_table_from_name text, p_table_to_name text, p_merge_partitions bool, p_where text DEFAULT NULL::text, p_delete_duplicates bool DEFAULT false)
	RETURNS int8
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
     ...
     v_where = v_where_cond || ' and ('||v_partition_key||'>='''||v_start_bdate|| '''::timestamp and '||v_partition_key||'<'''||v_end_bdate||'''::timestamp'||')';
       ...
       if p_merge_partitions = true then
        v_cnt_prt = fw.f_merge_tables(
         ...
         v_cnt_prt = fw.f_insert_table_sql(

-- DROP FUNCTION fw.f_load_simple_update(int8, text, text, text, bool);

CREATE OR REPLACE FUNCTION fw.f_load_simple_update(p_load_id int8, p_src_table text, p_trg_table text DEFAULT NULL::text, p_where text DEFAULT NULL::text, p_delete_duplicates bool DEFAULT false)
	RETURNS bool
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
DECLARE
  v_location      text := 'fw.f_load_simple_update';
  v_object_id     text;
  v_cnt           int8;
  v_src_table     text;
  v_trg_table     text;
...
v_cnt = fw.f_update_table_sql(
 ...
 perform fw.f_analyze_table(v_trg_table||'('||array_to_string(v_columns,',')||')');
 if  
     v_extr_type = 'DELTA' and v_delta_fld is not null then 
     v_end_date = least(fw.f_get_max_value(p_table_name:= v_src_table, p_field_name := v_delta_fld, p_where := v_where)::timestamp,v_end_date::timestamp);
 elseif   	
     v_extr_type = 'PARTITION' and v_bdate_fld is not null then 
     v_end_date = least(fw.f_get_max_value(p_table_name:= v_src_table, p_field_name := v_bdate_fld, p_where := v_where)::timestamp,v_end_date::timestamp);
 end if;

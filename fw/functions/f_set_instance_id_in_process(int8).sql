-- DROP FUNCTION fw.f_set_instance_id_in_process(int8);

CREATE OR REPLACE FUNCTION fw.f_set_instance_id_in_process(instance_id int8)
	RETURNS void
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
begin
	...
	v_sql = 'update fw.chains_info set status = ' || c_in_process_status::text ||', chain_start = '''||current_timestamp||''' where instance_id = ' || instance_id::text;

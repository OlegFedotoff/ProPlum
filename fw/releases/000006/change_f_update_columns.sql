DROP FUNCTION fw.f_update_columns(int8, text, _text, text, text);
CREATE OR REPLACE FUNCTION fw.f_update_columns(
    p_load_id int8, 
    p_table_from_name text, 
    p_column_list _text, 
    p_table_to_name text DEFAULT NULL::text, 
    p_date_field text DEFAULT NULL::text,
    p_terminate_lock boolean DEFAULT false
)
	RETURNS bool
	LANGUAGE plpgsql
	VOLATILE

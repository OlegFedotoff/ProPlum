DROP FUNCTION if exists fw.f_write_log(text, text, text, int8);

CREATE OR REPLACE FUNCTION fw.f_write_log(p_log_type text, p_log_message text, p_location text, p_load_id int8 DEFAULT NULL::bigint, p_log_user text DEFAULT ("current_user"())::text)
	RETURNS void
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
BEGIN
  ...
  v_sql := 'INSERT INTO fw.logs(log_id, load_id, log_type, log_msg, log_location, is_error,log_timestamp,log_user) 
            VALUES ( '||nextval('fw.log_id_seq')||' , '||coalesce(v_load_id, 0)||', '''||v_log_type||''', '||coalesce(''''||v_log_message||'''','''empty''')||', '||coalesce(''''||v_location||'''','null')||', '||case when v_log_type='ERROR' THEN true else false end||',current_timestamp,'''||coalesce(p_log_user,current_user)||''');';

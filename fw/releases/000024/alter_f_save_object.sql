-- Обновление функции f_save_object с добавлением поля param_list
-- DROP FUNCTION fw.f_save_object(fw.objects);

CREATE OR REPLACE FUNCTION fw.f_save_object(p_object_row fw.objects)
	RETURNS void
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
-- Permissions
ALTER FUNCTION fw.f_save_object(fw.objects) OWNER TO role_fw_owner;
GRANT ALL ON FUNCTION fw.f_save_object(fw.objects) TO public;
GRANT ALL ON FUNCTION fw.f_save_object(fw.objects) TO role_fw_owner;
GRANT ALL ON FUNCTION fw.f_save_object(fw.objects) TO role_data_loader;

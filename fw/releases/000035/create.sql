 ...
 perform fw.f_write_log(
  ...
  if v_cnt is not null then
     ...
     if v_cnt = 0 then -- in case of empty delta
        v_extract_to_value = v_extraction_from::text;
     else
        v_extract_to_value = coalesce(fw.f_get_max_value(v_tmp_table,v_date_field),v_extraction_to::text);
     end if;
  else
     v_res = false;
  end if;

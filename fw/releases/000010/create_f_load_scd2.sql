CREATE OR REPLACE FUNCTION fw.f_load_scd2(p_load_id int8, p_table_from text, p_table_to text DEFAULT NULL::text, p_ignore_attributes _text DEFAULT NULL::text[], p_primary_key _text DEFAULT NULL::text[], p_start_date_column text DEFAULT 'date_from'::text, p_end_date_column text DEFAULT 'date_to'::text, p_curr_date timestamp DEFAULT '2999-01-01 00:00:00'::timestamp without time zone)
    RETURNS bool
    LANGUAGE plpgsql
    VOLATILE
AS $$
 ...
 */
 declare
    v_location text := 'fw.f_load_scd2';
     v_object_id int8;
 ...
 select o.object_id, o.object_name
  from fw.objects o
  join fw.load_info li on o.object_id = li.object_id
 where li.load_id = p_load_id
  into v_object_id, v_table_to;
 v_where = fw.f_get_where_cond(p_load_id := p_load_id, p_table_alias:= 's');
 v_table_from = fw.f_unify_name(p_table_from);
 v_table_to = coalesce(fw.f_unify_name(p_table_to),fw.f_unify_name(v_table_to));
 v_curr_date  = coalesce(p_curr_date,'2999-01-01'::timestamp);
 v_stg_schema = coalesce(fw.f_get_constant('c_stg_table_schema'),'stg_')||fw.f_get_table_schema(v_table_to);
 v_primary_key = coalesce(p_primary_key, fw.f_get_merge_key(v_object_id));
 ...
 perform fw.f_set_load_id_success(p_load_id);

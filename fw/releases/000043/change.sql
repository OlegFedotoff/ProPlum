CREATE OR REPLACE FUNCTION fw.f_create_ext_table(p_table_name text, p_load_id bigint, p_load_method text DEFAULT NULL::text, p_connect_string text DEFAULT NULL::text, p_schema_name text DEFAULT NULL::text, p_prefix text DEFAULT NULL::text, p_suffix text DEFAULT NULL::text, p_is_writeble_type boolean DEFAULT false)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
	VOLATILE
...
BEGIN
  ...
  /*dvs65: for s3_archive writable tables map unbounded numeric -> numeric(38,2)
    so PXF/Parquet does not default to DECIMAL(38,18) (only 20 integer digits).
    Readable tables keep unbounded numeric to match parent for EXCHANGE PARTITION.*/
  ...
  --Нужно обрабатывать знаки доллара, s3 их не обрабатывает
  if v_load_method = 's3_archive' then
    --v_ext_t_name = replace(v_ext_t_name, '$', '');
    v_table_name = replace(v_table_name, '$', '');
    --v_table_name := v_table_name||'write'; --Параметр используем для создания папки с данными в S3
    if p_is_writeble_type then
      v_ext_t_name := v_ext_t_name||'write'; --Параметр используем для названия таблицы внешней таблицы на запись в S3
    else
      v_ext_t_name := v_ext_t_name||'read';  --Параметр используем для названия таблицы внешней таблицы на чтение в S3 (в дальнейшем таблица с этим названием при обмене партициями становится физической копией старой партиции
    end if;
  end if;
  ...
  if v_load_method = 's3_archive' then
     select string_agg(
            ...
            CASE
                ...
                WHEN data_type = 'numeric' AND p_is_writeble_type
                  -- PXF maps bare numeric to DECIMAL(38,18); (38,2) gives 36 integer digits
                  THEN 'numeric(38,2)'
